...
<?php
	class Bus{
		...
		public function busAdd($data){
			...
			$query="SELECT * from tbl_vehicle where busid='$busid'";
			$chkExist=$this->db->select($query);

			if($busid=="" || $nplate=="" || $seats==""|| $type==""){
				...
				$query="INSERT into tbl_vehicle(busid,nplate,seats,type) 
						values('$busid', '$nplate','$seats','$type')";
		...
		public function vechileLocationList(){
		...
		public function singleMessageList(){
			$query="SELECT * FROM tbl_adminchat GROUP BY(studentid) ORDER By created_at ASC";
			$getBus=$this->db->select($query);
			return $getBus;
		}

		public function busList(){
			$query="select * from tbl_vehicle order by busid ASC";
			$getBus=$this->db->select($query);
			return $getBus;
		}
		...
		public function delBusbyId($delid){
			...
			$query="Delete from tbl_vehicle WHERE busid='$delid'";
		...
		public function cancelBusTripById($busid, $date, $start, $reach){
	        ...
	        $updateTblBus="UPDATE tbl_vehicle 
	        		 SET
	        		 status='0'
	        		 WHERE busid='$busid'";
